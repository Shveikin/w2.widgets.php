...
// widgetwatcher__links.js

class widgetwatcher__links {
    ...
    */
    ...
    static add(path, key, watcher){
        if (!(path in widgetwatcher__links.global))
            widgetwatcher__links.global[path] = {}

        if (!(key in widgetwatcher__links.global[path]))
            widgetwatcher__links.global[path][key] = []


        widgetwatcher__links.global[path][key].push(watcher)
    }

    static update(path, key){
        if (!(path in widgetwatcher__links.global))
            return false;
        ...
        widgetwatcher__links.global[path][key].forEach(watcher => {
            watcher.update();
        });
    }
}
...
class widgetwatcher__update {
    ...
    get_value(widget = false){
        const _state = new widgetstate(this.path)
        const args = []
        
        this.keys.forEach(key => {
            args.push(_state.get(key))
        })

        const result = widgetconvertor.toFunction(this.callback).apply(widget, args)
        return result
    }
    ...
    update(){
        ...
        const value = this.get_value(is_func?false:this.to)
    ...
    update_widget(value){
        if (this.to){
            const changeProps = this.to.assignProp(this.props, value)
            if (changeProps)
...
class widgetwatcher extends widgetwatcher__update {
    static global = []
    ...
    constructor(path, keys, callback = false){
        ...
        const parser = widgetwatcher__tools.parsekeys(keys, callback)

        this.keys = parser.keys
        this.callback = parser.callback?parser.callback:this.tempcallback 

        this.keys.forEach(key => {
            widgetwatcher__links.add(this.path, key, this)
        });
        ...
        widgetwatcher.global.push(this);


        this.id = widgetwatcher.global.length;
    }
    ...
    link(to, props = false){
        ...
        // this._link = new widgetwatcher__link(this, to, props)
        // const watcherKey = this._link.key()

        // widgetwatcher.addToGlobal(this._path, watcherKey, this)
        this.update()

        return [this.path, this.id] //watcherKey]
    }
...
class widgetstate__props {
    ...
    static getdefault(path, key){
        if (
            widgetstate__props.issetPropName('defaults', path, key) &&
            'defaults' in widgetstate__props.props[path] && 
            key in widgetstate__props.props[path]['defaults']
        )
...
class widgetstate__tools extends widgetstate__static {
    ...
    method(method){
        return {
            apply: (bind, args) => {
                ...
                requeststorage.setDelay(this.props.delay || false)
    ...
    set(key, value){
        if (typeof value == 'object' && !key.startsWith('_')) {
            ...
            if (changed){
                ...
                widgetwatcher__links.update(this._path, key)
    ...
    change(key){
        const onchange = 'onchange' in this.props?this.props.onchange:false
        if (onchange){
            requeststorage.setDelay(this.props.delay || false)
    ...
    watch(keys, callback = false){
        return new widgetwatcher(this._path, keys, callback)
    }
...
class widget_smartprops {
    ...
    static dragboard(dragboard, props) {
        ...
        function mousemove(x, y){
            ...
            const axis = 'axis' in props?props.axis:false
...
class widget extends widget__tools {
    constructor(element = 'div', props = {}, root = false){
        super()
    ...
    __set(prop, value){
        if (prop=='child'){
            if (!('child' in this.props)) 
                this.props['child'] = []
            this.props.child.push(value)
        } else {
            this.props[prop] = value
        }
    }
    ...
    clearWatcherFrom(childs){
        childs.forEach(child => {
            ...
            child.watchlist.forEach(watcherprop => {
                ...
                // widgetwatcher.links[watcherprop[0]][watcherprop[1]] = null
                // delete widgetwatcher.links[watcherprop[0]][watcherprop[1]]
            })
